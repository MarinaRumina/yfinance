...
def get_multiple_histories(
    symbols: str = Query(
        ..., 
        description="Ticker symbols separated by commas (1-20 alphanumeric, may include . - ^ =)",
        pattern="^[A-Za-z0-9\\.\\^=-]{1,20}(,[A-Za-z0-9\\.\\^=-]{1,20})*$", # Это ограничение на ввод
        example="AAPL,^GSPC,ILS=X,LUMI.TA"
    ), 
